<template>
  <div ref="editorElement" class="markdown-content min-h-[600px] rounded-lg border border-neutral-200 bg-white dark:border-neutral-700 dark:bg-neutral-800"></div>
</template>
...
function editorJSToMarkdown(data: OutputData): string {
  ...
  for (const block of data.blocks) {
    switch (block.type) {
      case 'header':
        lines.push('#'.repeat(block.data.level) + ' ' + block.data.text)
        lines.push('')
        break
...
defineExpose({
  async insertShortcode(template: string) {
    ...
    await editor.blocks.insert('raw', {
      html: template,
    }, {}, blocks, true)
  },
})
...
:global(.dark) .ce-popover__item:hover {
  background-color: rgb(64 64 64);
}

/* Dark mode for code blocks */
:global(.dark) .cdx-block.ce-code__textarea {
