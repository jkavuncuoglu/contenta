<template>
  <div class="block-config-form">
    <div v-if="!block" class="text-center py-8">
      ...
      <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">Select a block</h3>
    ...
    <div v-else>
      ...
      <div class="mb-6 pb-4 border-b border-gray-200 dark:border-gray-600">
        ...
        <p v-if="block.description" class="text-sm text-gray-600 dark:text-gray-400 mt-1">
          {{ block.description }}
        </p>
      </div>
      ...
      <div class="space-y-6">
        ...
        >
          ...
          <div v-if="fieldConfig.type === 'string'">
            ...
            <select
              v-if="fieldConfig.options && fieldConfig.options.length > 0"
              :id="fieldName"
              ...
              @change="updateConfig"
            >
              <option value="">{{ fieldConfig.required ? 'Select...' : 'None' }}</option>
              <option
            ...
            <textarea
              ...
              v-model="localConfig[fieldName]"
              :required="fieldConfig.required"
              :placeholder="fieldConfig.default || `Enter ${fieldConfig.label || fieldName}`"
              rows="4"
              class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              @input="updateConfig"
            ></textarea>
            ...
            <p v-if="fieldConfig.description" class="mt-1 text-sm text-gray-500 dark:text-gray-400">
          ...
          <!-- Boolean Fields -->
          <div v-else-if="fieldConfig.type === 'boolean'" class="flex items-center">
            <input
              :id="fieldName"
              v-model="localConfig[fieldName]"
              type="checkbox"
            ...
            <p v-if="fieldConfig.description" class="ml-6 text-sm text-gray-500 dark:text-gray-400">
              {{ fieldConfig.description }}
            </p>
          </div>
          ...
          <div v-else-if="fieldConfig.type === 'array'">
            ...
            <input
              ...
              type="text"
              :required="fieldConfig.required"
              :placeholder="fieldConfig.default || 'Enter comma-separated values'"
              class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              @input="updateConfig"
            />
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
              {{ fieldConfig.description || 'Separate multiple values with commas' }}
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="mt-8 pt-6 border-t border-gray-200 dark:border-gray-600">
        <div class="flex items-center justify-between mb-4">
          <h4 class="text-md font-medium text-gray-900 dark:text-white">Live Preview</h4>
          <button
        ...
        <div class="border border-gray-200 dark:border-gray-600 rounded-lg p-4 bg-gray-50 dark:bg-gray-700">
          <BlockRenderer
            :section="{ id: 'preview', type: block.type, config: localConfig }"
...
const isLongText = (fieldName: string) => {
  return fieldName.includes('content') ||
...
const getInputType = (fieldName: string) => {
  ...
  if (fieldName.includes('url') || fieldName.includes('link') || fieldName.includes('image')) return 'url'
