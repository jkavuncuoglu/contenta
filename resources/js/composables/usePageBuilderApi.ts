...
export function usePageBuilderApi() {
  ...
  const updatePage = async (id: number, data: Partial<Page>): Promise<{ data: Page }> => {
  ...
  const previewPage = async (id: number): Promise<{ data: { html: string } }> => {
  ...
  const createLayout = async (data: Partial<Layout>): Promise<{ data: Layout }> => {
    const response = await axios.post(`${baseUrl}/layouts`, data)
    return response
  }

  const updateLayout = async (id: number, data: Partial<Layout>): Promise<{ data: Layout }> => {
  ...
  const createBlock = async (data: Partial<Block>): Promise<{ data: Block }> => {
    const response = await axios.post(`${baseUrl}/blocks`, data)
    return response
  }

  const updateBlock = async (id: number, data: Partial<Block>): Promise<{ data: Block }> => {
  ...
  const fetchBlockCategories = async (): Promise<{ data: Record<string, string> }> => {
    const response = await axios.get(`${baseUrl}/blocks-categories`)
    return response
  }

  const validateBlockConfig = async (blockId: number, config: any): Promise<{ data: { valid: boolean, errors: Record<string, string> } }> => {
    const response = await axios.post(`${baseUrl}/blocks/${blockId}/validate-config`, { config })
    return response
  }
