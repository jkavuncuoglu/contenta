...
const breadcrumbs: BreadcrumbItem[] = [
  ...
  {
    title: 'Edit Layout',
    href: `/admin/page-builder/layouts/${props.layout.id}/edit`,
  },
]
...
const form = reactive({
  ...
  slug: props.layout.slug,
  description: props.layout.description || '',
  is_active: props.layout.is_active,
  settings: {
...
// Initialize areas and settings from layout structure
onMounted(() => {
  const structure = props.layout.structure || {}
  areas.value = structure.areas || ['main']
  form.settings = {
    container_width: structure.settings?.container_width || 'full',
    spacing: structure.settings?.spacing || 'normal'
  }
})
...
const updateLayout = async () => {
  try {
    ...
    const response = await fetch(`/admin/page-builder/api/layouts/${props.layout.id}`, {
      method: 'PUT',
      headers: {
    ...
    if (response.ok) {
      ...
      console.error('Failed to update layout:', errors)
      alert('Failed to update layout. Please check the form and try again.')
    }
  } catch (error) {
...
<template>
  <Head :title="`Edit ${layout.name} - PageBuilder`" />
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Edit Layout</h1>
      ...
      <!-- Form -->
      <div class="max-w-4xl">
        <form @submit.prevent="updateLayout" class="space-y-8">
          <!-- Basic Information -->
          ...
          <div class="flex justify-end space-x-3">
            ...
            <button
              type="submit"
              :disabled="!form.name || !form.slug || areas.filter(a => a.trim()).length === 0 || saving"
