...
const toggleLayoutStatus = async (layout: Layout) => {
    try {
        const response = await fetch(
            `/admin/page-builder/api/layouts/${layout.id}`,
            {
                method: 'PUT',
                headers: {
...
const deleteLayout = async (layout: Layout) => {
    if (
        confirm(
            `Are you sure you want to delete "${layout.name}"? This may affect pages using this layout.`,
        )
    ) {
        try {
            const response = await fetch(
                            ...
                            >
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        ...
                                        :class="
                                            getStatusColor(layout.is_active)
                                        "
                                    >
                                        {{
                                            layout.is_active
