  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Comments</h1>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
          <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">Spam</h3>
          <p class="text-2xl font-bold text-red-600">{{ statistics.spam }}</p>
        </div>
        <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
          <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">Trash</h3>
          <p class="text-2xl font-bold text-gray-600">{{ statistics.trash }}</p>
        </div>
      </div>
      ...
      <!-- Bulk Actions -->
      <div v-if="selectedComments.length > 0" class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
        <div class="flex items-center justify-between">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
        <div v-if="!comments.length" class="p-12 text-center">
          <Icon icon="material-symbols-light:chat-bubble-outline" class="mx-auto h-12 w-12 text-gray-400" />
          <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">No comments</h3>
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">No comments match your current filters.</p>
        </div>
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center justify-between">
                  <div class="flex items-center space-x-2">
                    ...
                    <span v-if="comment.is_reply" class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-300">
                      Reply
                    </span>
                  </div>
                  <div class="flex items-center space-x-2">
                    ...
                    <div class="relative">
                      ...
                      >
                        <Icon icon="material-symbols-light:more-vert" class="w-4 h-4" />
                      </button>
                      <div
                ...
                <div class="mt-1">
                  <span class="text-xs text-gray-500 dark:text-gray-400">
                    On:
                    <a :href="`/posts/${comment.post_slug}`" class="text-blue-600 hover:text-blue-800">
                      {{ comment.post_title }}
                    </a>
                  </span>
                </div>
      ...
      <!-- Pagination -->
      <div v-if="pagination.last_page > 1" class="bg-white dark:bg-gray-800 px-4 py-3 rounded-lg shadow">
        <div class="flex items-center justify-between">
...
const updateCommentStatus = async (id: number, status: string) => {
  router.patch(`/admin/comments/${id}/status`, { status }, {
    preserveState: false,
...
const bulkUpdateStatus = async (status: string) => {
  ...
  router.patch('/admin/comments/bulk-status', {
