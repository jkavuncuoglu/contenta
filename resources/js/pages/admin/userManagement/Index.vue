        ...
        <div class="p-6">
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tr v-for="user in filteredUsers" :key="user.id">
                    ...
                    <td class="px-4 py-2 text-gray-500">{{ (user.roles || []).join(', ') }}</td>
                    <td class="px-4 py-2 flex gap-2">
                        <button @click="editUser(user)" class="bg-blue-500 hover:bg-blue-600 text-white px-2 py-1 rounded text-xs">Edit</button>
                        <button @click="archiveUser(user)" class="bg-yellow-500 hover:bg-yellow-600 text-white px-2 py-1 rounded text-xs">Archive</button>
                        <button @click="deleteUser(user)" class="bg-red-500 hover:bg-red-600 text-white px-2 py-1 rounded text-xs">Delete</button>
                    </td>
                </tr>
                </tbody>
            </table>
