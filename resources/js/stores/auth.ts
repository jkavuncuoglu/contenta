...
const user = ref<any>(typeof window !== 'undefined' ? (window as any).__page?.props?.user ?? null : null);
...
export function useAuthStore() {
  ...
  const getTwoFactorRecoveryCodes = async () => {
    try {
      const res = await fetch('/auth/two-factor/recovery-codes', { credentials: 'same-origin' });
      if (!res.ok) return { success: false };
      const data = await res.json();
  ...
  const getTwoFactorQrCode = async () => {
    try {
      const res = await fetch('/auth/two-factor/qrcode', { credentials: 'same-origin' });
      if (!res.ok) return { success: false };
      const data = await res.json();
  ...
  const enableTwoFactor = async () => {
    try {
      const res = await fetch('/auth/two-factor/enable', { method: 'POST', credentials: 'same-origin' });
      if (!res.ok) return { success: false };
      const data = await res.json();
  ...
  const regenerateTwoFactorRecoveryCodes = async () => {
    try {
      const res = await fetch('/auth/two-factor/recovery-codes/regenerate', { method: 'POST', credentials: 'same-origin' });
      if (!res.ok) return { success: false };
      const data = await res.json();
      return { success: true, data: data.data ?? data };
    } catch (e) {
  ...
  const disableTwoFactor = async () => {
    try {
      const res = await fetch('/auth/two-factor/disable', { method: 'POST', credentials: 'same-origin' });
      if (!res.ok) return { success: false };
      const data = await res.json();
      return { success: true, data };
    } catch (e) {
