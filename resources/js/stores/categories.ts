    ...
    return new Promise(async (resolve) => {
      try {
        inertia.get('/admin/categories', params, {
          preserveState: true,
          onSuccess: (page: any) => {
            const res = (page.props && (page.props.categories ?? page.props.data ?? page.props)) ?? {};
            const data = res.data ?? res ?? [];
            categories.value = Array.isArray(data) ? data : data.data ?? [];
            resolve({ success: true, data: categories.value });
          },
          onError: (page: any) => {
            const msg = (page.props && (page.props.flash?.error || page.props.error || page.props.message)) ?? 'Failed to load categories';
    ...
    return new Promise((resolve) => {
      try {
        inertia.post('/admin/categories', payload, {
          preserveState: true,
          onSuccess: (page: any) => {
            const res = (page.props && (page.props.category ?? page.props.data ?? page.props)) ?? {};
            resolve({ success: true, data: res.data ?? res });
          },
          onError: (page: any) => {
            const msg = (page.props && (page.props.flash?.error || page.props.error || page.props.message)) ?? 'Failed to create category';
            error.value = typeof msg === 'string' ? msg : JSON.stringify(msg);
            resolve({ success: false, error: error.value });
          }
        });
    ...
    return new Promise((resolve) => {
      try {
        inertia.put(`/admin/categories/${id}`, payload, {
          ...
          onError: (page: any) => {
            const msg = (page.props && (page.props.flash?.error || page.props.error || page.props.message)) ?? 'Failed to update category';
            error.value = typeof msg === 'string' ? msg : JSON.stringify(msg);
            resolve({ success: false, error: error.value });
          }
        });
