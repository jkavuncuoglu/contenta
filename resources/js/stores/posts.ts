...
export function usePostsStore() {
  ...
  const fetchPost = async (id: number): Promise<StoreResult> => {
    ...
    return new Promise((resolve) => {
      try {
        inertia.get(`/admin/posts/${id}`, {}, {
          preserveState: true,
          onSuccess: (page: any) => {
            const res = (page.props && (page.props.post ?? page.props.data ?? page.props)) ?? {};
            currentPost.value = res.data ?? res ?? null;
            resolve({ success: true, data: currentPost.value });
          },
          onError: (page: any) => {
            const msg = (page.props && (page.props.flash?.error || page.props.error || page.props.message)) ?? 'Failed to fetch post';
    ...
    return new Promise((resolve) => {
      try {
        inertia.post('/admin/posts', payload, {
          ...
          onError: (page: any) => {
            const msg = (page.props && (page.props.flash?.error || page.props.error || page.props.message)) ?? 'Failed to create post';
            error.value = typeof msg === 'string' ? msg : JSON.stringify(msg);
            resolve({ success: false, error: error.value });
          }
        });
